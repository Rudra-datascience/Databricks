...
users_data = spark.createDataFrame(data=users)
...
df = users_data.withColumnRenamed("id","Emp ID").withColumn("ingestion_Date",current_timestamp())
...
users_data\
.withColumnRenamed("id", "Emp ID")\
.withColumn("ingestion_date", current_timestamp())\
.withColumn("Full_Name", concat(col("name"), lit(" "), col("last_name")))\
.drop("is_customer", "name", "last_name").display()
